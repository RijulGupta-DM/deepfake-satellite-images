...
The DM-AER-DeepFake-V1 dataset is split into sample, validation, and training folder.s The real images are a part of the [Million-AID][million-aid] dataset. These images were not present during the creation of the synthetic images. The synthetic images are generated by DeepMedia. The sample folder contains images from the validation set, but there is no overlap in images between the validation and training sets. The data can be downloaded at this google drive location.
